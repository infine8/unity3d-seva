...
public class TexturePaintScript : MonoBehaviour
{
    ...
    private Texture2D _textureSnapshot;
    ...
    void Start()
    {
        ...
        else
        {
            ...
            if (useMyMaterial)
            {
                materialTarget = renderer.material;
            }
    ...
    public void MakeTextureSnapshot()
    {
        RenderTexture.active = colorRenderTextureDestination;

        _textureSnapshot = new Texture2D(colorRenderTextureDestination.width, colorRenderTextureDestination.height);
        _textureSnapshot.ReadPixels(new Rect(0, 0, colorRenderTextureDestination.width, colorRenderTextureDestination.height), 0, 0);
        _textureSnapshot.Apply();
    }
    ...
    {
        if (_textureSnapshot == null) _textureSnapshot = startingColorTextureSource;
        ClearColorBufferToTexture(_textureSnapshot);
    }


    //========

    public void ClearColorBufferToTexture()
    {
        _textureSnapshot = null;
        ClearColorBufferToTexture(startingColorTextureSource);
    }
